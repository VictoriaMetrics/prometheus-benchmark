...
- It generates production-like workload for both data ingestion and querying paths:
  - It generates [Prometheus](https://prometheus.io/docs/concepts/metric_types/) write workload from real [node_exporter](https://github.com/prometheus/node_exporter) metrics.
  ...
  - (Optional) It generates [OpenTelemetry](https://opentelemetry.io/docs/specs/otel/metrics/) workload from [Host Metrics Receiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/receiver/hostmetricsreceiver/README.md).
...
For Prometheus workload, [vmagent](https://docs.victoriametrics.com/victoriametrics/vmagent/) scrapes metrics from [node_exporter](https://github.com/prometheus/node_exporter)
...
For OpenTelemetry workload, [OTel Collector](https://opentelemetry.io/docs/collector/) generates load using [Host Metrics Receiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/receiver/hostmetricsreceiver/README.md) and pushes the scraped metrics to the configured Prometheus-compatible remote storage systems. These systems must support [OpenTelemetry Protocol](https://opentelemetry.io/docs/specs/otlp/)
for measuring data ingestion performance. 
Optionally these systems may support
...
- `otel-collector` - this pod generates load using the host metrics receiver and writes to storage via OTLP.
